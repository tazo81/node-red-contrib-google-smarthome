...
class GoogleSmartHome extends Aggregation(Auth, Devices, HttpAuth, HttpActions, Emitter) {
    constructor(username, password, httpsPort, ssl, publicKey, privateKey, jwtkey, clientid, clientsecret, reportStateInterval, debug) {
        super();
    ...
    Start() {
        try {
            ...
            if (this._ssl) {
                ...
                this.httpServer = stoppable(require('https').createServer(httpsOptions, this.app), graceMilliseconds);
            } else {
                // create our HTTP server
                this.httpServer = stoppable(require('http').createServer(this.app), graceMilliseconds);
            }
